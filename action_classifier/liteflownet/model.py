...
class Network(torch.nn.Module):
	def __init__(self):
		...
		self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.load("action_classifier/liteflownet/network-default.pytorch").items() })
	# end
...
def estimate(tenFirst, tenSecond,Network):
	'''global netNetwork
	...
	tenFlow = torch.nn.functional.interpolate(input=Network(tenPreprocessedFirst, tenPreprocessedSecond), size=(intHeight, intWidth), mode='bilinear', align_corners=False)
